...
d3.csv("nobel_laureates.csv").then(data => {
    ...
    const stemCategories = ["medicince", "physics", "chemistry"];
    const categorizedData = data.map(d => ({
        ...d,
        categoryGroup: stemCategories.includes(d.category) ? "STEAM" : "Non-STEM"
    }));
    ...
    const colorScale = d3.scaleOrdinal() // typically used for categorical data
